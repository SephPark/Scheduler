	...
	public PagingUtil(String keyfield, String keyword, int currentPage, int totalCount, int rowCount,
		...
		if (currentPage > pageCount) {
			if(keyword==null){//검색 미사용시
				pagingHtml.append("<ul class='pagination'><li><a href="+pageUrl+"?pageNum="+ (startPage - 1) + addKey +">");
			}else{
				pagingHtml.append("<ul class='pagination'><li><a href="+pageUrl+"?keyfield="+keyfield+"&keyword="+keyword+"&pageNum="+ (startPage - 1) + addKey +">");
			}
			pagingHtml.append("≪");
			pagingHtml.append("</a><li>");
		}else {
			pagingHtml.append("<ul class='pagination'><li><span>≪</span></li>");
		}
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (i == currentPage) {
				pagingHtml.append("<li class='active'><span>");
				pagingHtml.append(i);
				pagingHtml.append("</span></li>");
			} else {
				if(keyword==null){//검색 미사용시
		...
		if (totalPage - startPage >= pageCount) {
			...
			pagingHtml.append("<li><span>≫</span></li></ul>");
